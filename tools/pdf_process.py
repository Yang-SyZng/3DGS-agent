...
_pdf_process: PDFProcess | None = None
_pdf_process_tools: list[Callable[..., Any]] | None = None
...
class LazyPDFProcess:
    def extract_pdf_stats(self, pdf_path: str) -> dict[str, Any]:
        return get_PDFProcess().extract_pdf_stats(pdf_path=pdf_path)

    def detect_pdf_type(self, pdf_path: str) -> dict[str, Any]:
        return get_PDFProcess().detect_pdf_type(pdf_path=pdf_path)
    ...
    def detect_layout_blocks(
        ...
        return get_PDFProcess().detect_layout_blocks(
            pdf_path=pdf_path,
            max_pages=max_pages,
        )
    ...
    def extract_text(
        ...
        return get_PDFProcess().extract_text(
            pdf_path=pdf_path,
            pdf_type=pdf_type,
            use_ocr_for_mixed=use_ocr_for_mixed,
        )
    ...
    def parse_pdf(self, pdf_path: str, use_ocr_for_mixed: bool = False) -> dict[str, Any]:
        return get_PDFProcess().parse_pdf(
            pdf_path=pdf_path,
            use_ocr_for_mixed=use_ocr_for_mixed,
        )
...
pdf_process = LazyPDFProcess()


def get_PDFProcessTools() -> list[Callable[..., Any]]:
    global _pdf_process_tools

    if _pdf_process_tools is None:
        process = get_PDFProcess()
        _pdf_process_tools = [
            process.extract_pdf_stats,
            process.detect_pdf_type,
            process.detect_layout_blocks,
            process.extract_text,
            process.parse_pdf,
        ]

    return _pdf_process_tools


PDFProcessTools = LazyToolList(get_PDFProcessTools)
